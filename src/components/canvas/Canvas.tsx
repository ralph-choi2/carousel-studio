...
import type { CarouselPage, CoverData, PageData } from '@/lib/types';
...
export function Canvas({
  ...
  const isThumbPreview = Boolean(b2bPreview);
    ...
    >
      <div className="flex items-center justify-center" style={{ width: 56, flexShrink: 0 }}>
        <Button
          ...
          onClick={onPrev}
          disabled={isFirst || isThumbPreview}
      ...
      <div className="flex flex-1 items-center justify-center min-w-0">
        {isThumbPreview && b2bPreview ? (
          <B2bThumbPage data={b2bPreview} scale={scale * 2.5} />
        ) : page && Component ? (
          <Component
            data={page.data}
            styles={page.styles}
      ...
      <div className="flex items-center justify-center" style={{ width: 56, flexShrink: 0 }}>
        <Button
          ...
          className="rounded-full text-white/60 hover:text-white hover:bg-white/10"
          onClick={onNext}
          disabled={isLast || isThumbPreview}
      ...
      >
        {isThumbPreview ? 'B2B 썸네일 (미리보기)' : `${currentIndex + 1} / ${pages.length}`}
      </div>
    </div>
  );
}
