---
name: comfy-image
description: fal.ai openai/gpt-image-2 (medium) + gws CLI 로컬 스크립트로 캐러셀 배경 이미지 생성 후 Drive 저장 + 시트 URL 기록. Triggers on "이미지 생성", "배경 만들어", "comfy", "ComfyUI", "이미지 채워".
---

# Carousel Image Generation (fal.ai 로컬 스크립트)

캐러셀 cover / scene-card 배경 이미지를 `scripts/generate-images.ts` 로컬 스크립트로 생성한다. 스크립트 내부에서 fal.ai `openai/gpt-image-2` (OpenAI GPT-Image 2.0, `quality:"medium"` + `image_size:"portrait_4_3"`) API 직접 호출 → 로컬 tmp 저장 → `gws drive files create` 로 Shared Drive 공용 폴더 적재 + anyone 권한 부여 → 공개 URL 을 시트 셀 `pages[idx].data.bg_image` 에 기록.
...
pages[].data.bg_prompt
  ↓ fal.ai openai/gpt-image-2 (image_size:"portrait_4_3", quality:"medium", 페이지 병렬)
fal CDN URL (임시)
  ↓ 바이너리 다운 → /tmp/{key}.png
PNG 파일
...
- `3:4`, `portrait`, `1080x1350`, `aspect ratio` 등 비율/해상도 지정 문구
- `no text`, `no watermark`, `no letters` 등 네거티브 프롬프트

이유: 스크립트가 `image_size:"portrait_4_3"` + `quality:"medium"` 파라미터로 정확히 지정한다. 프롬프트 중복은 오히려 모델을 혼란시켜 품질 저하.
...
출력 크기: **1024×1536**. 에디터는 CSS `object-fit: cover` 로 1080×1350 캔버스 맞춤.

단가: **$0.05/장** (portrait_4_3 medium). 월 100장 ≈ $5. `quality:"high"` 로 올리면 $0.17/장 (3.4배), 현재 용도엔 과함. nano-banana-2 ($0.08) 대비 38% 저렴 + 품질 우수로 2026-04-22 교체.
